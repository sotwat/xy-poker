...
### [確認] 1. データベース（Supabase）の接続状況
Supabaseはクラウドサービスであるため、データベースの移行作業やSQLの再実行は**不要**です。すでに本番データベース上でテーブル `ai_global_parameters` が稼働し、最強パラメータが格納されています。新環境では、後述の `.env` ファイルの設定のみで自動的に接続されます。
