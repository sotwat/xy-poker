...
`npm run deploy` requires `XYPOKER_FONT_PACK` in the ignored `.env.local` file or the process environment. It prepares `.private/vectura.bin` for the Pages Function. The font is never copied into `dist` or Git. `npm run build` still works without the private pack; use `npm run prepare:lettering` followed by `npx wrangler pages dev dist` to check the complete local production runtime. Public font use must satisfy [the creator's terms](docs/font-selection.md). Unselected comparison images stay local and can be regenerated with `scripts/render_home_lettering.py`.
...
1. **v09051209** (2026-09-05): **Certified Endgame A8** - Added exhaustive final-placement checks to the default game AI. A7's move changes only when every possible hidden board preserves or improves the win/draw/loss result, or when every possible final opponent reply loses. The solver shares the 1,000 ms deadline and discards incomplete certificates. An independent 2,000-game endgame audit found 39 certified replacements and 11 improved outcomes with no deterioration; trajectories were generated by lightweight A7, so this is not a full-game win-rate claim. Retained A7's priors and rollouts after a broader candidate failed its independent adoption gate. Added exhaustive reducer-oracle tests, hidden-information checks, reproducible analysis, and A8 record metadata. See [the research report](GTO_A8_REPORT.md).
